...
- (void)setTextValue:(CGFloat)value
{
    ...
    {
        self.text = [NSString stringWithFormat:@"%@%%",self.formatBlock(value)];
    }
    else
    {
        ...
        if([self.format rangeOfString:@"%(.*)d" options:NSRegularExpressionSearch].location != NSNotFound || [self.format rangeOfString:@"%(.*)i"].location != NSNotFound )
        {
            ...
            NSString *string2 = @"%";
            NSString *string3 = [NSString stringWithFormat:@"%@%@",string,string2];
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:string3];
            [str addAttribute:NSForegroundColorAttributeName value:AssistColor range:NSMakeRange(0,string.length)];
            [str addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Arial-BoldItalicMT" size:35.0] range:NSMakeRange(0, string.length)];
